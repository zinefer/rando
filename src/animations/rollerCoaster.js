...
export function rollerCoaster({ elements, newOrder, positions, gridDimensions, gridRect, sticky, timeline }) {
  ...
  if (animatableIndices.length === 0) {
    console.log('[rollerCoaster] No cards to animate based on settings');
    return;
  }
  ...
  newOrder.forEach((itemIndex, newIndex) => {
    ...
    if (!shouldAnimateCard(itemIndex, sticky)) {
      console.log(`[rollerCoaster] Card ${itemIndex} skipping animation based on settings`);
      return;
    }
    ...
    const carPosition = newIndex % animatableIndices.length;
